...
# 공마다 스피드, 크기, 방향이 다 다르므로 딕셔너리로 관리
# 최초 발생하는 제일 큰 공 추가
balls.append({
    "pos_x": 50,  # 공의 x 좌표
    "pos_y": 50,  # 공의 y 좌표
...
while running:
    ...
    # 공 위치 정의
    # enumerate(리스트) => 리스트에 있는 인덱스 번호와 값을 가져와 출력
    for ball_idx, ball_val in enumerate(balls):
        ...
        if ball_pos_y >= screen_height - stage_height - ball_height:
            ball_val["to_y"] = ball_val["init_spd_y"]  # 처음 바닥에 닿았을땐 최초 속도로 설정
        # 스테이지에 닿고 올라가는 속도가 점차 줄다가 다시 바닥으로 떨어지는 처리
        else:
            ball_val["to_y"] += 0.5
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        break  # 안쪽 for문에서 break을 만나면 여기로 진입 가능. 2중 for문을 한번에 탈출

    # 충돌된 공 or 무기 없애기
    if ball_to_remove > -1:
        del balls[ball_to_remove]  # 리스트 안의 인덱스에 해당되는 공이 삭제
        ball_to_remove = -1
